...
Usage:
    ...
    python gq_scripts/preprocess/convert_nii_to_npz.py \
        --dataset btcv \
        --img-dir /home/gaoqi/dataset/using/btcv_1/test/img \
        --gt-dir /home/gaoqi/dataset/using/btcv_1/test/gt \
        --output-dir /home/gaoqi/BiomedParse/gq_data/btcv/test

    # PROMISE12 示例
    python gq_scripts/preprocess/convert_nii_to_npz.py \
    --dataset promise12 \
    --img-dir /home/gaoqi/dataset/using/promise12_2/test/img \
    --gt-dir /home/gaoqi/dataset/using/promise12_2/test/gt \
    --output-dir /home/gaoqi/BiomedParse/gq_data/promise12/test
"""
import os
import argparse
import glob
...
def get_img_gt_pairs(img_dir: str, gt_dir: str, dataset: str) -> list[tuple[str, str]]:
    ...
    for img_path in img_files:
        ...
        if dataset == "acdc":
            ...
            # BTCV (nnU-Net): case_00001_00000.nii.gz -> case_00001.nii.gz
            match = re.match(r"^(.+)_\d{5}\.nii\.gz$", base)
            if match:
                gt_name = match.group(1) + ".nii.gz"
            # BTCV (id-Image/id-Mask): 0507688-Image.nii.gz -> 0507688-Mask.nii.gz
            elif name.endswith("-Image"):
                gt_name = name.replace("-Image", "-Mask") + ".nii.gz"
            else:
                gt_name = base
        elif dataset == "promise12":
            # PROMISE12: CaseXX.nii.gz -> CaseXX_gt.nii.gz 或 CaseXX_segmentation.nii.gz
            for candidate in (f"{name}_gt.nii.gz", f"{name}_segmentation.nii.gz", base):
                gt_path_cand = os.path.join(gt_dir, candidate)
                if os.path.exists(gt_path_cand):
                    gt_name = candidate
                    break
            else:
                gt_name = base
        ...
        if not os.path.exists(gt_path) and gt_name != base and dataset != "promise12":
            # 回退：尝试同名文件
