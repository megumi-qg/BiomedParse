...
# Import metric computation functions from evaluate_v2
from gq_scripts.evaluate_v2 import (
    load_prompts,
    select_prompts,
    compute_hd95,
    compute_nsd,
...
def inference_2d_slice(model, image_pil, prompts_list):
    ...
    for prompt in prompts_list:
        ...
        # pred_mask_prob is (H, W) probability map
...
def merge_2d_masks(pred_mask_probs_list, class_ids, threshold=0.5):
    ...
    """
    H, W = pred_mask_probs_list[0].shape
    merged_mask = np.zeros((H, W), dtype=np.int32)
    
    # Stack all probability maps
    prob_stack = np.stack(pred_mask_probs_list, axis=0)  # (num_classes, H, W)
