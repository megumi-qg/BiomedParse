"""
convert_camus_to_npz.py

Convert CAMUS `.nii.gz` image and gt pairs to the BiomedParse `.npz` format.
...
Usage example:
    python biomedparse_datasets/convert_camus_to_npz.py \
      --dataset-root data/CAMUS --split test --output-root data/CAMUS

--dataset-root: /home/gaoqi/dataset/using/camus2
--split: test_2d
--output-root: /home/gaoqi/BiomedParse/data/CAMUS（输出到 BiomedParse 的数据目录）
...
def detect_depth_axis(shape):
    ...
    # For CAMUS 2D ultrasound, depth axis might be different
    # If 2D image (H, W), return None (no depth axis)
    if len(shape) == 2:
        return None
...
def process_case(img_path, gt_path):
    ...
    # Handle 2D images (CAMUS ultrasound images are typically 2D)
    if imgs.ndim == 2:
    ...
    if depth_axis is None:
        ...
        # Use original zooms directly, not zooms3
        if len(zooms) >= 2:
...
def run(dataset_root, split, output_root, pattern="*.nii.gz"):
    ...
    for img_path in img_files:
        ...
        # CAMUS naming: patient0027_2CH_ED.nii.gz -> patient0027_2CH_ED_gt.nii.gz
...
def make_prompts_template(output_data_root, dataset_name="CAMUS"):
    # create a minimal class_prompts.json template for CAMUS
    # CAMUS typically has: 0=background, 1=Left Ventricle Cavity, 2=Myocardium, 3=Left Atrium
    import json
    ...
    # Add or update CAMUS prompts
    prompts[dataset_name] = {
        "1": ["Left ventricle cavity", "Left ventricle cavity in cardiac ultrasound"],
        "2": ["Myocardium", "Myocardium in cardiac ultrasound"],
        "3": ["Left atrium", "Left atrium in cardiac ultrasound"],
        "instance_label": 0
    }
...
def parse_args():
    parser = argparse.ArgumentParser(description="Convert CAMUS .nii.gz files to BiomedParse .npz format")
